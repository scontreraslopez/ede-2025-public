...
- [PARTE 2 — UP11: PRUEBAS AUTOMÁTICAS](#parte-2--up11-pruebas-automáticas)
  ...
  - [4. Dobles de prueba](#4-dobles-de-prueba)
    - [4.1. Dos problemas, dos tipos de dobles](#41-dos-problemas-dos-tipos-de-dobles)
    - [4.2. Ampliación: Cada tipo con un ejemplo](#42-ampliación-cada-tipo-con-un-ejemplo)
  ...
  - [6. Más allá de las pruebas unitarias](#6-más-allá-de-las-pruebas-unitarias)
...
#### Ampliación: Gradle y Maven

**Gradle** y **Maven** son las herramientas de construcción (*build tools*) estándar en el ecosistema Java/Kotlin. En cualquier proyecto profesional o de código abierto las encontrarás: no son una opción, son la infraestructura sobre la que se construye todo lo demás.

Su función es centralizar y automatizar el ciclo de vida del proyecto:

- **Gestión de dependencias**: declaran las librerías que necesita el proyecto (incluido JUnit) y las descargan automáticamente desde repositorios como Maven Central.
- **Compilación y empaquetado**: compilan el código y generan el artefacto final (`.jar`, `.war`...).
- **Ejecución de tests**: lanzan toda la suite de pruebas con un único comando (`mvn test` / `./gradlew test`) y generan informes de resultados.

Este último punto es el que hace que las *build tools* sean el eslabón clave entre los tests y la **integración continua**: cuando GitHub Actions, Jenkins o cualquier otra herramienta de CI arranca un pipeline, lo que ejecuta internamente es exactamente ese mismo comando. Sin una *build tool* bien configurada, los tests no pueden ejecutarse de forma desatendida, y sin eso, la CI no existe. Es decir, **una buena suite de tests sin build tool es como tener un buen motor sin transmisión**: el trabajo está hecho, pero no llega a ningún sitio.

- **Maven** usa `pom.xml` (XML) y un ciclo de vida fijo y predecible (`compile → test → package → install`).
- **Gradle** usa `build.gradle.kts` (Kotlin DSL), más conciso y rápido gracias a su caché incremental. Es el estándar en proyectos Kotlin y Android.

Para usar JUnit 5 basta con declararlo como dependencia de test y habilitar JUnit Platform en la tarea de test (ver sección 2.1).

> En este módulo trabajamos con proyectos IntelliJ standalone para centrarnos en el diseño y escritura de tests, pero en cualquier proyecto real el primer paso es tener la *build tool* configurada correctamente.

**¿Quieres profundizar?**

Os dejo algunos recursos para entender mejor el papel de Gradle y Maven en el proceso de testing si os interesa.

- 🎬 [Gradle Tutorial — Crash Course](https://www.youtube.com/watch?v=gKPMKRnnbXU) — Marco Codes. Cubre el wrapper, `build.gradle`, plugins y el ciclo de construcción.
- 🎬 [Maven Tutorial — Nice & Easy](https://www.youtube.com/watch?v=Xatr8AZLOsE) — Marco Codes. Explica `pom.xml`, fases del ciclo de vida y gestión de dependencias.
- 📄 [Apache Maven Tutorial](https://www.baeldung.com/maven) — Baeldung. Referencia escrita muy completa para consultar cuando lo necesites.
...
La primera línea importa el **BOM** (*Bill of Materials*) de JUnit, que centraliza la gestión de versiones. Así, el resto de artefactos de JUnit no necesitan especificar versión — Gradle las resuelve automáticamente de forma consistente. Es la forma recomendada cuando el proyecto puede crecer y añadir más módulos de JUnit.

Ten en cuenta que la versión del BOM puede variar, así que es **recomendable consultar Maven Central para obtener la última versión estable**.
...
Muchos de estos ejemplos se entienden mejor si miras las actividades resueltas de esta unidad y lo haces de manera simultánea en tu IDE. Te animo a que lo hagas: la teoría es importante, pero la práctica es donde realmente se asienta el conocimiento.
...
Si te gusta, puedes crear una plantilla en IntelliJ para generar automáticamente el esqueleto Given-When-Then cada vez que creas un nuevo test.
...
Para simplificar el código de los tests, se pueden importar estáticamente todos los métodos de `Assertions`:
...
Las pruebas unitarias prueban **una sola unidad en aislamiento**. El problema es que en la práctica las clases tienen dependencias: bases de datos, servicios externos, otras clases. Si para probar `ArrivalChecker` necesito tener en marcha una BD real y un servidor web, el test será lento, frágil y difícil de reproducir.

Los **dobles de prueba** (*test doubles*) son objetos que sustituyen a esas dependencias durante el test. Como los especialistas de acción en el cine: no son los actores reales, pero hacen el trabajo que se necesita en la escena.

#### 4.1. Dos problemas, dos tipos de dobles

Antes de ver los cinco tipos concretos, hay que entender que hay **dos problemas distintos** que pueden surgir al tener dependencias:

**Problema A — La SUT *recibe* datos de la dependencia** (p.ej. consulta una BD). Necesito controlar qué datos recibe para poder probar distintos escenarios sin depender de datos reales.

**Problema B — La SUT *envía* datos a través de la dependencia** (p.ej. envía un email, escribe en un log). No hay valor de retorno que asertar; tengo que verificar que la llamada se realizó correctamente.

Según cuál sea el problema, usaremos un tipo distinto de doble:

| Tipo | Resuelve | Descripción en una frase |
| ---- | -------- | ------------------------ |
| **Dummy** | Ninguno (caso trivial) | Se pasa para satisfacer una firma, nunca se llama |
| **Stub** | Problema A | Devuelve valores fijos; el test no verifica cómo se usó |
| **Fake** | Problema A | Tiene lógica real simplificada; cada test puede configurar sus datos |
| **Mock** | Problema B | El test verifica que se llamó con los argumentos correctos; falla si no |
| **Spy** | Problema B | Como Mock pero envuelve un objeto real y conserva su comportamiento |

#### 4.2. Ampliación: Cada tipo con un ejemplo

A continuación se desarrolla en un ejemplo cada tipo de doble, usando el mismo escenario para facilitar la comparación. El uso de dobles de prueba es una habilidad que se adquiere con la práctica, así que lo mejor es que leas los ejemplos y luego los implementes tú mismo en tu IDE para entenderlos a fondo. Este contenido puede considerarse como de **ampliación**, más allá de que puntualmente necesitemos crear un Stub/Fake/Mock para resolver un test concreto.

Usamos siempre el mismo escenario: `ArrivalChecker` depende de `RouteRepository` (lee rutas del disco) y de `Logger` (registra eventos).

**Dummy** — la dependencia es obligatoria en la firma pero irrelevante para este test:

```kotlin
// Logger es requerido por el constructor, pero este test no prueba el logging
val dummyLogger = object : Logger {
    override fun log(msg: String) { /* vacío — nunca se llamará */ }
}
val checker = ArrivalChecker(stubRepo, train, dummyLogger)
// Podemos probar la lógica de llegada sin que el Logger nos moleste
```

**Stub** — la SUT *lee* de la dependencia; controlo qué datos recibe:

```kotlin
// RouteRepository real lee ficheros del disco → lento y frágil en tests
// Stub: devuelve siempre la misma ruta fija, sin ninguna lógica interna
val stubRepo = object : RouteRepository {
    override fun getRoute(id: String): Route = Route(listOf("Alicante", "Elche", "Murcia"))
}
val checker = ArrivalChecker(stubRepo, train, dummyLogger)
checker.check()
// Ahora asertar sobre el resultado de ArrivalChecker con datos controlados
assertEquals("Elche", checker.nextStop())
```

**Fake** — igual que Stub, pero cuando distintos tests necesitan datos distintos:

```kotlin
// Stub siempre devuelve lo mismo. Si necesito distintos escenarios, uso un Fake.
// Fake: guarda y recupera rutas de verdad, pero en memoria (sin disco)
class FakeRouteRepository : RouteRepository {
    private val routes = mutableMapOf<String, Route>()
    fun add(id: String, route: Route) { routes[id] = route }
    override fun getRoute(id: String): Route = routes[id] ?: error("Ruta no encontrada: $id")
}

// En el test:
val fakeRepo = FakeRouteRepository()
fakeRepo.add("R1", Route(listOf("Alicante", "Elche")))
// Otro test puede añadir "R2" con datos completamente distintos
```

> [!TIP]
> **Stub vs Fake**: el Stub hardcodea un único valor para todos los tests. El Fake tiene lógica real y puede responder distinto según qué le pidas. Empieza con Stub; crea un Fake cuando el Stub se quede corto.

**Mock** — la SUT *escribe* a través de la dependencia; verifico que lo hizo bien:

```kotlin
// Logger mock (con Mockito, ver sección 4.3): graba todas las llamadas que recibe
val mockLogger = mock<Logger>()
val checker = ArrivalChecker(stubRepo, train, mockLogger)
checker.check()

// VERIFICACIÓN: ¿se llamó a log() exactamente con este mensaje?
verify(mockLogger).log("Llegada confirmada en: Elche")
// Si no se llamó, o se llamó con otro mensaje → el test FALLA
```

**Spy** — quiero el comportamiento real, pero también verificar las llamadas:

```kotlin
// Spy sobre el Logger real: ejecuta su lógica original Y registra las llamadas
val spyLogger = spy(ConsoleLogger())
val checker = ArrivalChecker(stubRepo, train, spyLogger)
checker.check()

// El log se escribió de verdad en consola (comportamiento real conservado)
// Y además podemos verificar que se llamó
verify(spyLogger).log(any())
```

> [!TIP]
> **Mock vs Spy**: el Mock reemplaza la dependencia por completo (no ejecuta nada real). El Spy la envuelve: la lógica original se ejecuta, y encima podemos verificar las llamadas. Usa Spy cuando necesitas el comportamiento real pero también quieres comprobar interacciones.
...
```kotlin
// ¡IMPORTANTE! En build.gradle.kts:
...
Si te interesa profundizar en Mockito, te recomiendo el siguiente recurso: [Dominando las Pruebas Unitarias en JAVA | Mockito](https://www.youtube.com/watch?v=0VivFIbTZ3c). Incluso si no usas Mockito, este vídeo es un buen repaso al tema de Testing con JUnit en IntelliJ.
...
Además del **TDD**, podemos encontrar también el **ATDD** (*Acceptance Test Driven Development*), que extiende el enfoque a pruebas de aceptación a nivel de sistema. En ATDD, los tests se escriben desde la perspectiva del usuario final o del cliente, y se centran en validar que el sistema cumple con los requisitos funcionales. Herramientas como Cucumber permiten escribir estos tests en lenguaje natural (Gherkin), facilitando la colaboración entre desarrolladores, testers y stakeholders.

Un ejemplo de test de aceptación **ATDD** con Cucumber podría ser:

```gherkin
Feature: Gestión de estaciones

  Scenario: Crear una nueva estación
    Given el usuario tiene permisos de administrador
    When el usuario crea una estación con nombre "Alicante" y velocidad de aproximación 30 m/s
    Then la estación "Alicante" se muestra en la lista de estaciones
```
...
> [!NOTE]
> Esta sección desarrolla la visión más allá del alcance práctico del módulo. Se trata de una ampliación teórica sin entrar en demasiada profundidad, para que quede claro el lugar que ocupan las pruebas unitarias dentro del panorama general de testing y tengáis una visión completa de las herramientas y técnicas disponibles en la industria.
...
**Herramientas destacadas:** A continuación se mencionan algunas de las herramientas más populares en cada categoría. Esta lista no es exhaustiva, ni voy a evaluarlo, su propósito es simplemente daros una visión general de lo que hay ahí fuera.
...
A nivel conceptual: **CI** (*Continuous Integration*) consiste en integrar el código con mucha frecuencia en un repositorio compartido; **CD** (*Continuous Delivery/Deployment*) consiste en desplegar con pocos clics, o de forma totalmente automática, una nueva versión de la aplicación.

Todo esto cobra su máximo valor cuando está integrado en un pipeline de CI/CD (Jenkins, GitHub Actions, GitLab CI): cada push dispara automáticamente toda la suite de tests, y si algo falla, el merge queda bloqueado hasta que se corrija. **El testing unitario es la base sobre la que descansa CI**, y este a su vez el pilar sobre el que se apoya la CD.
