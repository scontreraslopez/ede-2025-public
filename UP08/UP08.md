...
![Diagrama mostrando el sistema como rectángulo que contiene casos de uso representados como elipses](image.png)
...
![Diagrama mostrando relación de inclusión (include) donde Realizar pedido siempre incluye Validar usuario con flecha discontinua](image-1.png)
...
![Diagrama mostrando relación de extensión (extend) donde Aplicar descuento opcionalmente extiende Realizar pedido con flecha discontinua](image-2.png)
...
Se pueden etiquetar los extremos de la asociación para aclarar el papel que juega cada clase en la relación:
...
**Roles en código Java:**

Los roles se traducen directamente a **nombres de atributos** en las clases. Veamos diferentes casos:

**Asociación unidireccional (Persona → Empresa):**

```java
public class Persona {
    private String nombre;
    private Empresa empleador;  // Rol: empleador

    public Persona(String nombre) {
        this.nombre = nombre;
    }

    public void setEmpleador(Empresa empresa) {
        this.empleador = empresa;
    }
...
**Asociación bidireccional (Persona ↔ Empresa):**
...
public class Persona {
    ...
    public void setEmpleador(Empresa empresa) {
        ...
        if (empresa != null && !empresa.getEmpleados().contains(this)) {
...
public class Empresa {
    private String nombre;
    private List<Persona> empleados;  // Rol: empleados
...
> **Importante**: En asociaciones bidireccionales, hay que tener cuidado de mantener la **consistencia** de la relación en ambos sentidos. Una mala práctica común es que al modificar un lado de la relación, el otro quede desincronizado.
...
**Ejemplo:** Un equipo de fútbol tiene jugadores, pero los jugadores pueden existir sin el equipo (pueden cambiar de equipo).
...
public class Jugador {
    ...
    // El jugador puede existir sin equipo
}
...
public class Equipo {
    ...
    public void traspasarJugador(Jugador jugador) {
        jugadores.remove(jugador);
        // El jugador sigue existiendo después de salir del equipo
    }
}
...
barcelona.ficharJugador(messi);  // Messi se une al equipo
barcelona.traspasarJugador(messi);  // Messi deja el equipo pero sigue existiendo

Equipo psg = new Equipo("PSG");
psg.ficharJugador(messi);  // El mismo jugador puede unirse a otro equipo
```

**Característica clave**: Los objetos Jugador se crean **fuera** del Equipo y pueden sobrevivir a la eliminación del Equipo.
...
public class LineaPedido {
    ...
    // Constructor package-private: solo Pedido puede crear líneas
...
public class Pedido {
    ...
    public Pedido(int numero) {
        this.numero = numero;
        this.lineas = new ArrayList<>();  // El Pedido crea y posee las líneas
    }

    public void agregarLinea(String producto, int cantidad, double precio) {
        // El Pedido crea las líneas internamente
        LineaPedido linea = new LineaPedido(producto, cantidad, precio);
        lineas.add(linea);
    }
    ...
    // Cuando se elimina el pedido, las líneas también desaparecen
    public void cancelar() {
        lineas.clear();  // Las líneas se destruyen con el pedido
    }
}

// Uso:
Pedido pedido = new Pedido(12345);
pedido.agregarLinea("Laptop", 1, 999.99);
pedido.agregarLinea("Mouse", 2, 25.00);

// No se pueden crear LineaPedido fuera de Pedido
// LineaPedido linea = new LineaPedido(...);  // Error: constructor no accesible

pedido.cancelar();  // Al cancelar el pedido, las líneas se eliminan
```

**Característica clave**: Las líneas de pedido se crean **dentro** del Pedido y su ciclo de vida está completamente controlado por él. Cuando el Pedido desaparece, las líneas también.
...
// El servicio DEPENDE de ClienteDTO, pero no lo almacena
public class ClienteServicio {
    private ClienteRepository repository;

    // Dependencia tipo 1: Parámetro de método
    public Cliente crearCliente(ClienteDTO dto) {
        // Se usa el DTO temporalmente para crear el Cliente
        ...
        // El DTO ya no se usa después de este método
    }

    // Dependencia tipo 2: Objeto local creado temporalmente
    public void enviarNotificacion(Cliente cliente) {
        EmailMessage mensaje = new EmailMessage();  // Dependencia local
        mensaje.setDestinatario(cliente.getEmail());
        mensaje.setAsunto("Bienvenido");
        mensaje.setContenido("Gracias por registrarte");

        EmailSender.send(mensaje);  // Dependencia tipo 3: método estático
        // El objeto mensaje ya no existe fuera de este método
    }
}
```

**Característica clave**: La clase dependiente NO almacena referencias permanentes a la clase de la que depende. La relación solo existe durante la ejecución de un método.
...
#### 7. Acoplamiento y relaciones entre clases

El **acoplamiento** (coupling) mide el grado de dependencia entre clases. Es uno de los conceptos más importantes en el diseño de software orientado a objetos.

**¿Qué es el acoplamiento?**

El acoplamiento indica **cuánto conoce una clase sobre otra** y **cuánto depende de ella**. Cuanto más acopladas están dos clases, más probable es que un cambio en una requiera cambios en la otra.

**Niveles de acoplamiento:**

```text
ACOPLAMIENTO BAJO (deseable)           ACOPLAMIENTO ALTO (problemático)
        ↓                                           ↓
   Dependencia → Asociación → Agregación → Composición → Herencia
```

**1. Dependencia (acoplamiento más bajo)**

```java
public class ReporteServicio {
    public void generarReporte(List<Cliente> clientes) {
        // Solo usa Cliente temporalmente, no lo almacena
...
**Ventajas**: Mínima dependencia, fácil de cambiar.
**Cuándo usarla**: Para operaciones puntuales que no requieren relación permanente.

**2. Asociación (acoplamiento bajo-medio)**

```java
public class Pedido {
    private Cliente cliente;  // Asociación: conoce al cliente

    public Pedido(Cliente cliente) {
        this.cliente = cliente;  // Recibe el cliente desde fuera
    }
}
```

**Ventajas**: Flexible, los objetos son independientes.
**Cuándo usarla**: Cuando las clases tienen ciclos de vida independientes.

**3. Agregación (acoplamiento medio)**
...
public class Departamento {
    ...
    public void agregarEmpleado(Empleado emp) {
        empleados.add(emp);  // Empleado existe independientemente
    }
}
```

**Ventajas**: Relación todo-parte manteniendo independencia.
**Cuándo usarla**: Para colecciones donde las partes pueden existir solas.

**4. Composición (acoplamiento alto)**
...
public class Casa {
    ...
    public Casa() {
        ...
        habitaciones.add(new Habitacion("Salón"));  // Casa crea las habitaciones
    }
}
// Si se destruye Casa, las Habitaciones también desaparecen
```

**Desventajas**: Las partes no pueden reutilizarse, fuerte dependencia.
**Cuándo usarla**: Cuando las partes no tienen sentido sin el todo.

**5. Herencia (acoplamiento MUY alto)**

```java
public class Coche extends Vehiculo {
    // Coche conoce TODA la implementación interna de Vehiculo
    // Un cambio en Vehiculo puede romper Coche
}
```

**Desventajas**: Máximo acoplamiento, cambios en la superclase afectan a todas las subclases.
**Cuándo usarla**: Solo para relaciones "es-un" genuinas, preferir composición cuando sea posible (principio "Composition over Inheritance").

**Relación con los principios SOLID (UP07):**

El acoplamiento está directamente relacionado con los principios que estudiaste en la unidad anterior:

- **Dependency Inversion Principle (DIP)**: Reduce el acoplamiento dependiendo de abstracciones (interfaces) en lugar de implementaciones concretas.

```java
// ALTO acoplamiento: depende de implementación concreta
public class Pedido {
    private MySQLDatabase db;  // ❌ Acoplado a MySQL
}

// BAJO acoplamiento: depende de abstracción
public class Pedido {
    private Database db;  // ✅ Puede ser cualquier implementación
}
```

- **Open/Closed Principle (OCP)**: Un diseño con bajo acoplamiento es más fácil de extender sin modificar código existente.

**¿Acoplamiento alto es siempre malo?**

No necesariamente. El acoplamiento es inevitable y necesario. Lo importante es:

1. **Ser consciente** del nivel de acoplamiento que introduces
2. **Minimizarlo** donde sea posible sin sobre-complicar el diseño
3. **Concentrarlo** en lugares estratégicos (ej: una clase Factory que concentra las dependencias)
4. **Usar el tipo de relación adecuado** para cada situación

**Ejemplo práctico: Evolución del diseño**

```java
// ❌ MAL: Alto acoplamiento innecesario
public class CarritoCompra {
    private PagoConTarjeta procesadorPago;  // Acoplado a una implementación

    public void pagar() {
        procesadorPago.procesarTarjeta();
    }
}

// ✅ MEJOR: Bajo acoplamiento usando interfaz (DIP)
public class CarritoCompra {
    private ProcesadorPago procesadorPago;  // Depende de abstracción

    public void pagar() {
        procesadorPago.procesar();  // Puede ser tarjeta, PayPal, etc.
    }
}
...
class PagoConTarjeta implements ProcesadorPago {
    public void procesar() { /* ... */ }
}

class PagoConPayPal implements ProcesadorPago {
    public void procesar() { /* ... */ }
}
```
...
El acoplamiento es una métrica de calidad del diseño. Un buen diseño:
- Minimiza el acoplamiento entre clases no relacionadas
- Maximiza la cohesión dentro de cada clase
- Usa el nivel de acoplamiento apropiado para cada relación
- Favorece la dependencia de abstracciones sobre implementaciones concretas
